...
func (p *GoogleProvider) updateLanguageCode(ctx context.Context, languageMap map[string]string, voicesToKeep map[string]interfaces.Voice) error {
	languageCount := make(map[string]int)
	alreadyAddedLanguageCode := map[string]bool{}
	var languageCodesToAdd []interfaces.LanguageCode

	// Get existing language codes from Firestore to check for name changes
	existingLangCodes := make(map[string]interfaces.LanguageCode)
	...
	for _, doc := range docs {
		var lc interfaces.LanguageCode
	...
	for langCode := range uniqueLanguageCodes {
		...
		if existingLC, exists := existingLangCodes[langCode]; exists {
			...
			if _, exists = alreadyAddedLanguageCode[langCode]; !exists {
				...
				languageCodesToAdd = append(languageCodesToAdd, interfaces.LanguageCode{
					Code:     langCode,
					Country:  country,
					Language: langName,
					Name:     name,
	...
	if len(languageCodesToAdd) > 0 {
		...
		err := AddToFirestore(ctx, p.firestoreClient, "languageCodes", languageCodesToAdd, func(lc interfaces.LanguageCode) string {
			return lc.Code
		})
		if err != nil {
