...
func (p *GoogleProvider) processAndFilterVoices(languageMap map[string]string, voicesToKeep map[string]interfaces.Voice,
	existingLanguages, existingVoices map[string]bool) ([]interfaces.Voice, []interfaces.Voice, []interfaces.Language, error) {
	var googleVoices []interfaces.Voice
...
func mapGender(ssmlGender texttospeechpb.SsmlVoiceGender) interfaces.Gender {
	gender := interfaces.MALE
	if ssmlGender == texttospeechpb.SsmlVoiceGender_NEUTRAL {
		gender = interfaces.NEUTRAL
	}
...
func (p *GoogleProvider) CreateSampleMP3(ctx context.Context, voice interfaces.Voice, langName string, outputDir string) error {
	...
	voiceReq := &texttospeechpb.VoiceSelectionParams{
		...
		Name:         voice.Name,
		SsmlGender:   texttospeechpb.SsmlVoiceGender(int32(voice.SsmlGender)),
	}
	...
	if err := os.WriteFile(outputFile, resp.AudioContent, 0600); err != nil {
