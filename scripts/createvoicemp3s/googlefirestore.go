...
func (p *GoogleProvider) addNewRecordsToFirestore(ctx context.Context, voicesToAdd []interfaces.Voice, languagesToAdd []interfaces.Language) error {
	...
	if len(languagesToAdd) > 0 {
		...
		err := AddToFirestore(ctx, p.firestoreClient, "languages", languagesToAdd, func(l interfaces.Language) string {
			return l.Code
		})
		if err != nil {
